...
def index(request):
    ...
    active_tasks = len(SimpleOrder.objects.filter(status__value='active'))
    employees = len(Performer.objects.all())
    customers = len(Customer.objects.all())
    return render(request, 'index.html', {"performers": performers, "active_tasks": active_tasks,
