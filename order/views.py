...
class EditOrderView(View):
    ...
    def get(self, request):
        ...
        order = get_object_or_404(SimpleOrder, id=id)
        form_data = {"description": order.description, "name": order.name, "time_end": order.time_end.strftime("%Y-%m-%d %H:%M:%S"),
                     "price_low": order.price_low, "price_high": order.price_high,
                     "is_fixed_price": order.is_fixed_price, "payment": order.payment,
                     "category": order.category.value, "subcategory": order.subcategory.value,
                     "order_image_preview":order.order_image_preview}
        simple_order_form = SimpleOrderForm(data=form_data)
